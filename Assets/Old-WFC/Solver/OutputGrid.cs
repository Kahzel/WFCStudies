...
namespace WaveFunctionCollapse
{
    public class OutputGrid
    {
        ...
        {
            return JaggedArrayFactory.ValidateCoordinates(position.x, position.y, width, height);
        }
        ...
        {
            int[][] returnGrid = JaggedArrayFactory.CreateJaggedArray<int[][]>(this.height, this.width);
            if (!CheckIfGridIsSolved())
            {
                return JaggedArrayFactory.CreateJaggedArray<int[][]>(0, 0);
            }
            for (int row = 0; row < this.height; row++)
            {
                for (int col = 0; col < this.width; col++)
